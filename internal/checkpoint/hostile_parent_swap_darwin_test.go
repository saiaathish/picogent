...
func TestDarwinSameUIDCheckpointParentSwapConfinement(t *testing.T) {
	if runtime.GOOS != "darwin" {
		t.Skip("darwin-only same-UID checkpoint parent-swap harness")
	}
	...
	for _, op := range ops {
		...
		cmd := exec.Command(os.Args[0], "-test.run", "^TestDarwinSameUIDCheckpointParentSwapAttackerHelper$", "-test.count=1")
...
func TestDarwinSameUIDCheckpointParentSwapAttackerHelper(t *testing.T) {
