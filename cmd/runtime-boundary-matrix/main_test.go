...
func TestRunRetainsExternalArtifact(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not installed")
	}
	workspace := t.TempDir()
	...
	artifact := filepath.Join(t.TempDir(), "runtime-boundary-matrix.json")
	var stdout, stderr bytes.Buffer
	...
	loaded, err := runtimeboundary.LoadReport(artifact, sha)
	if err != nil {
		t.Fatal(err)
	}
	if loaded.CandidateSHA != sha {
		t.Fatalf("loaded sha = %s", loaded.CandidateSHA)
	}
	inside := filepath.Join(workspace, "inside-matrix.json")
	code = run(context.Background(), []string{
		"--workspace", workspace,
		"--candidate-sha", sha,
		"--out", inside,
	...
	if code == 0 {
		t.Fatal("expected inside-workspace artifact to fail")
	}
}
