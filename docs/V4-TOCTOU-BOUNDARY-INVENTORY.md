...
behavior tip is 4086b1f3416cde476025718139aa2c8260cc9d8e after PR #723, and
the exact-head evidence refresh is tracked by child #724. The evidence
...
PR #723 narrowed one evolution-store coordination edge: after preserving the
non-creating first-use path, existing state loads acquire the shared store
lock before reading the payload. This is bounded hardening only; it does not
make the pre-lock existence probes and payload read a universal atomic
...
| Evolution store state | `internal/evolve/store.go` preserves a non-creating first-use load by probing the parent and state directory, then uses `acquireStoreLock` before `loadLocked` reads an existing payload. `securefile.EnsureDir`, `ReadFile`, and `WriteAtomic` retain their bounded file contract; the shared `securefile.OpenLockFile` / `LockFile` contract owns lock bootstrap and platform locking. The old raw `internal/evolve/atomic*.go` and platform lock helpers were removed. | Evolution-store tests cover ordinary persistence, cross-process serialization, Unix rejection of symlinked state parents, state files, and lock files, plus a deterministic regression that a concurrent `Load` waits for the shared lock before reading existing state; securefile platform tests cover the underlying descriptor/handle implementations. | The secure-file contract and load lock narrow pathname replacement, symlink, and stale-read windows but do not provide an atomic cross-process snapshot across the initial existence probes, lock acquisition, and payload read. The broad same-UID TOCTOU claim remains `UNVERIFIED`. |
