...
| Session, task, goal, and trace state | Active session/delete-undo, task-state, goal, and trace payload reads/writes use `securefile` plus application locks. Directory enumeration and multi-record workflows still use ordinary path operations at their coordination edges. | Bounded file limits, atomic publication, lock ownership, restart recovery, and state validation are covered by their package tests. | The secure-file payload contract does not make directory enumeration or multi-file transactions an atomic cross-process snapshot. The legacy `internal/session/atomic.go` helper is a separate raw pathname implementation and should remain out of any security claim until its live ownership is confirmed. |
| Evolution store state | `internal/evolve/store.go` uses `os.Stat`, `os.ReadFile`, `os.MkdirAll`, and `internal/evolve/atomic.go` stages with `os.CreateTemp` before platform `replaceFile`; Unix/Windows store locks use `os.OpenFile` on a pathname. | Atomic publication and evolution-store tests cover ordinary persistence and cross-process serialization. | Temp creation, cleanup, parent sync, lock bootstrap, load, and final replacement remain pathname-based in this surface. The current hostile workspace evidence must not be projected onto evolution state. |
