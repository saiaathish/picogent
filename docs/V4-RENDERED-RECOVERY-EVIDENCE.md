...
| Sequence | Boundary | Directly observed rendered state | Filesystem corroboration |
| --- | --- | --- | --- |
| 1 | Fresh seed page before the prompt | Page `336` loaded in Safe mode with no undo control. | Seed manifest recorded the probe's expected initial state as `absent`. |
| 2 | Safe-mode permission gate | The DOM showed the fixture prompt, `Changed files (0)`, and `Deny`, `This turn`, `Always allow`, and `Allow` controls. The permission card identified `write rendered-recovery-probe.txt`. | No pre-allow filesystem command was durably recorded; the seed manifest was created from a fresh contained workspace with `before=absent`. |
| 3 | After clicking `Allow` | The DOM showed `Edited 1 file`, `Changed files (1)`, and an enabled `Undo last change` control. | `rendered-recovery-probe.txt` existed with 26 bytes and the expected SHA-256 `6edfb9937f622dadd7cb093d2e4150c3747de904ef6be0e15d15eed69e1e627b`. |
| 4 | After clicking `Undo last change` | The DOM no longer exposed the recovery banner or undo control; the task projection was `Verifying`. The rendered transcript visibly recorded `Undid last turn: removed rendered-recovery-probe.txt`. | The probe path was absent. The historical `Changed files (1)` summary remained, which is the prior-turn history rather than evidence that the file still exists. |
| 5 | Fresh reload process | Page `337` loaded the same project, task, prior prompt, and mutation history. The task remained `Verifying`, `Changed files (1)` remained as history, and no stale recovery banner or undo control was exposed. | The probe path remained absent after the fresh process loaded the durable state. |
