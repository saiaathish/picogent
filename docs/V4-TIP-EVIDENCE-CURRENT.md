...
The exact-head packet records `live-provider-connectivity=PASS` and
`live-provider-quality=PASS`, with matrix summary PASS 8 / INCONCLUSIVE 1 /
UNVERIFIED 3. Rendered-platform rows remain `UNVERIFIED`, broad hostile
filesystem TOCTOU remains `UNVERIFIED`, and release authorization remains
`INCONCLUSIVE` without an operator decision. The older live-provider
